...
The first build for a target compiles the vendored libc from source and caches it; subsequent builds are incremental like everything else. `import std;` and C++ modules work identically on every target — the `std` module and libc++ objects are built per-target in the cache. musl and windows targets link statically by default; gnu targets always link dynamically.
...
Only the libraries you actually use end up in `DT_NEEDED` (a hello world links just `libc.so.6`). Static linking is rejected on gnu targets (glibc's static mode has known dlopen/NSS defects) — use a musl target for static builds.

## Sanitizers

`bake cc` / `bake c++` and `bake build` (`[profile.*] sanitize = [...]`) support `-fsanitize=address` and `-fsanitize=undefined`. The runtimes are built from bake's vendored compiler-rt and cached per target on first use. On macos the runtime is a shared library loaded from the cache at run time, so sanitized binaries need the cache directory in place; on linux it links statically. On windows-gnu, ubsan links statically and asan is x86_64-only, with the runtime library copied next to the executable.

Sanitizers are a native development tool: native builds are the contract; cross-built sanitized products are not validated. Other sanitizers (tsan, msan, …) are unsupported — compile-only invocations still instrument, but the link is rejected with a clear message.
