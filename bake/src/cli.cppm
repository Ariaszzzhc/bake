...
export void print_version() { std::println("bake {}", BAKE_VERSION); }
...
void merge_manifest_config(MoidDeclaration &decl, const Manifest &manifest,
  ...
  std::string current_triple =
      tc.target.is_native() ? detect_host_target().triple_ : tc.target.triple_;
...
std::expected<void, std::string>
configure_moid_graph(MoidGraph &graph, const Toolchain &tc, const Path &out_dir,
  ...
  for (const auto &id : *topology) {
    ...
    if ((moid_dir / "build.cpp").is_regular_file()) {
      ...
      if (!cache_hit)
        declaration =
            compile_and_run_build_cpp(moid_dir, *manifest, node, tc, out_dir);
    } else {
    ...
    merge_manifest_config(declaration, *manifest, tc, profile_name);
...
export int cmd_build(const ParsedArgs &args) {
  ...
  // Cross-compile target from --target=<triple> or -t <triple>.
  {
    ...
    if (target_spec) {
      ...
      tc.target = parse_target(*target_spec);
    }
  }
  ...
  auto configured =
      configure_moid_graph(*outer_graph, tc, out_dir, *root, profile);
  if (!configured) {
    std::println(std::cerr, "bake: {}", configured.error());
    return 1;
  }

  auto graph = build_graph(*outer_graph, tc, out_dir, *root, prebuilt_modules);
  if (!graph) {
