...
// Atomically publish a completed sibling file over its destination.
export inline std::error_code atomic_replace_file(
        const Path& source, const Path& destination) {
#ifdef _WIN32
    if (!MoveFileExW(source.fs().wstring().c_str(),
                     destination.fs().wstring().c_str(),
    ...
    std::filesystem::rename(source.fs(), destination.fs(), error);
    return error;
#endif
}
